...
func (b *Background) CheckStockPriceInBG(outChan chan<- Response, ticker, traderID, author, expiry string, guildID string, exit chan bool) {
	...
	dbStock, err := b.Repo.GetStock(ticker, traderID)
	if err != nil {
	...
	for {
		...
		case <-tick.C:
			if !db.IsTradingHours() {
				if dbStock.ChannelType == discord.DAY || !expiryDate.IsZero() && time.Now().After(expiryDate) {
					outChan <- Response{
						Type: Expired,
					}
					return
